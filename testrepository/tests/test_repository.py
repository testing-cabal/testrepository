...
class FailingCase:
    def run(self, result):
        ...
        result.addError(self, None, details={"traceback": content.text_content("tb")})
...
class TestRepositoryErrors(ResourcedTestCase):
    def test_not_found(self):
        ...
        self.assertEqual(
            'No repository found in %s. Create one by running "testr init".' % url,
            str(error),
        )
...
class TestRepositoryContract(ResourcedTestCase):
    ...
    def test_open_non_existent(self):
        url = "doesntexistatall"
        self.assertThat(
            lambda: self.repo_impl.open(url), raises(repository.RepositoryNotFound(url))
        )
    ...
    def test_latest_id_empty(self):
        repo = self.repo_impl.initialise(self.sample_url)
        self.assertThat(repo.latest_id, raises(KeyError("No tests in repository")))
    ...
    def test_get_failing_partial_runs_preserve_missing_failures(self):
        ...
        self.assertEqual(
            set(["passing", "missing"]), set([test[0].id() for test in analyzed.errors])
        )
    ...
    def test_get_test_run_missing_keyerror(self):
        ...
        result.stopTestRun()
        inserted = result.get_id()
        self.assertThat(lambda: repo.get_test_run(inserted - 1), raises(KeyError))
    ...
    def test_get_test_run_preserves_time(self):
        ...
        result.status(
            timestamp=(now + 1 * second), test_id=test_id, test_status="success"
        )
        inserted = result.stopTestRun()
        run = repo.get_test_run(inserted)
    ...
    def test_get_failing_get_subunit_stream(self):
        ...
        legacy_result = testtools.ExtendedToStreamDecorator(result)
        legacy_result.startTestRun()
        make_test("testrepository.tests.test_repository.Case.method", False).run(
            legacy_result
        )
        legacy_result.stopTestRun()
        run = repo.get_failing()
    ...
    def test_get_subunit_from_test_run(self):
        ...
        legacy_result = testtools.ExtendedToStreamDecorator(result)
        legacy_result.startTestRun()
        make_test("testrepository.tests.test_repository.Case.method", True).run(
            legacy_result
        )
        legacy_result.stopTestRun()
        inserted = result.get_id()
    ...
    def test_inserted_test_times_known(self):
        ...
        legacy_result.stopTestRun()
        self.assertEqual({test_name: 0.1}, repo.get_test_times([test_name])["known"])
