...
class TestCommand(ResourcedTestCase):
    ...
    def get_test_ui_and_cmd(
        self, options=(), args=(), proc_outputs=(), proc_results=()
    ):
        self.dirty()
        ui = UI(
            options=options,
    ...
    def test_no_config_settings_errors(self):
        ...
        self.assertThat(
            ui.outputs[0][1],
            MatchesException(
                ValueError("No test_command option present in .testr.conf")
            ),
        )
    ...
    def test_IDLIST_failures(self):
        ...
        self.assertEqual(
            True, cmd.repository_factory.repos[ui.here].get_test_run(1)._partial
        )
    ...
    def test_load_list_failing_takes_id_intersection(self):
        ...
        self.useFixture(
            MonkeyPatch(
                "testrepository.testcommand.TestCommand.get_run_command", capture_ids
            )
        )
    ...
    def test_load_list_passes_ids(self):
        ...
        list_file.flush()
        ui, cmd = self.get_test_ui_and_cmd(options=[("load_list", list_file.name)])
        cmd.repository_factory = memory.RepositoryFactory()
        self.setup_repo(cmd, ui)
        self.set_config(
            "[DEFAULT]\ntest_command=foo $IDOPTION\ntest_id_option=--load-list $IDFILE\n"
    ...
    def test_partial_passed_to_repo(self):
        ui, cmd = self.get_test_ui_and_cmd(options=[("quiet", True), ("partial", True)])
        cmd.repository_factory = memory.RepositoryFactory()
        self.setup_repo(cmd, ui)
        self.set_config("[DEFAULT]\ntest_command=foo\n")
        result = cmd.execute()
    ...
    def test_process_exit_code_nonzero_causes_synthetic_error_test(self):
        ...
        self.assertEqual([Wildcard, "fail"], [test["status"] for test in run._tests])
    ...
    def test_regex_test_filter(self):
        ...
        self.assertThat(params[0][2], MatchesListwise([Equals("bar"), Equals("quux")]))
    ...
    def test_force_init(self):
        ...
        self.assertEqual(
            [
                ("values", [("running", "foo ")]),
                ("popen", ("foo ",), {"shell": True, "stdin": PIPE, "stdout": PIPE}),
                ("results", Wildcard),
