...
class TestCLIUI(ResourcedTestCase):
    ...
    def test_error_enters_pdb_when_TESTR_PDB_set(self):
        ...
        ui = cli.UI([], stdin, stdout, stderr)
        ui.output_error(err_tuple)
        self.assertThat(stderr.getvalue(), DocTestMatches(expected, doctest.ELLIPSIS))
...
""",
    ...
    def test_outputs_summary_to_stdout(self):
        ...
        self.assertEqual(
            ("%s\n" % (expected_summary,)).encode("utf8"), ui._stdout.buffer.getvalue()
        )
    ...
    def test_parse_options_after_double_dash_are_arguments(self):
        ...
        ui.set_command(cmd)
        self.assertEqual(
            {"doubledash": ["--"], "myargs": ["one"], "subargs": ["--two", "three"]},
            ui.arguments,
        )
...
class TestCLISummary(TestCase):
    ...
    def test_values_no_delta(self):
        x = self.get_summary(
            True, None, None, None, None, [("failures", 12, None), ("errors", 13, None)]
        )
    ...
    def test_combination(self):
        ...
        self.assertEqual(
            (
                ...
                "PASSED (failures=12 (-1), errors=13 (+2))"
            ),
            x,
        )
...
class TestCLITestResult(TestCase):
    ...
    def make_result(self, stream=None, argv=None, filter_tags=None):
        ...
        ui.set_command(cmd)
        return ui.make_result(lambda: None, StubTestCommand(filter_tags=filter_tags))
    ...
    def test_addFailure_handles_string_encoding(self):
        ...
        result.status(
            ...
            file_name="traceback",
            mime_type="text/plain;charset=utf8",
            file_bytes=b"-->\xe2\x80\x9c<--",
            eof=True,
        )
        pattern = "...-->?<--..."
        self.assertThat(
            stream.getvalue().decode("utf8"), DocTestMatches(pattern, doctest.ELLIPSIS)
        )
